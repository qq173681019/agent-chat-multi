...
const WS_URL = `ws://localhost:${config.serverPort || 3000}`;
...
function connect() {
  ...
  ws.on('open', () => {
    console.log(`[Agent] 连接成功，身份: ${BOT_NAME}`);
  ...
  ws.on('close', () => { console.log('[Agent] 断开，重连中...'); setTimeout(connect, 3000); });
  ws.on('error', (e) => console.error('[Agent] 错误:', e.message));
}
...
console.log(`[Agent] 启动中... (${BOT_NAME} / ${MODEL})`);
connect();
